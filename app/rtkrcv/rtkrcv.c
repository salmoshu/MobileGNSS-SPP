...
#include "../user_inc/vt.h"
#include "../user_inc/user_parameter.h"
#include "../user_inc/at_command.h"
...
static const char *helptxt[]={
    ...
    "rover                 : start rover station",
    "base                  : start base station",
    ""
};
...
static int startsvr(vt_t *vt)
{
    ...
    /* start rtk server */
    vt_printf(vt,"station: %s\n",prcopt.stationType == 1 ? "ROVER":prcopt.stationType == 2 ?"BASE":"ERROR");
...
static void prstatus(vt_t *vt)
{
    ...
    vt_printf(vt,"%-28s: %s %s\n","rtkrcv version",VER_RTKLIB,PATCH_LEVEL);
...
/* rover station command --------------------------------------------------------------*/
static void cmd_rover(char **args, int narg, vt_t *vt)
{
    trace(3,"rover station start:\n");
    prcopt.stationType = 1;
    if (!startsvr(vt)) return;
    vt_printf(vt,"rover station start\n");
}
/* base station command --------------------------------------------------------------*/
static void cmd_base(char **args, int narg, vt_t *vt)
{
    trace(3,"base station start:\n");
    prcopt.stationType = 2;
    if (!startsvr(vt)) return;
    vt_printf(vt,"base station start\n");
}
/* console thread ------------------------------------------------------------*/
    ...
    while (con->state) {
        ...
        switch (j) {
            ...
            case 19: /* shutdown */
                ...
                break;
            case 20: cmd_rover    (args,narg,con->vt); break;
            case 21: cmd_base   (args,narg,con->vt); break;
            default:
...
int main(int argc, char **argv)
{
    ...
    char *dev="",file[MAXSTR]="../../conf/rtkrcv/rover.conf";
    int deamon=0;
