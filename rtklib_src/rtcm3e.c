...
/* encode MSM header ---------------------------------------------------------*/
static int encode_msm_head_new(int type, rtcm_t *rtcm,rtcm_t* temp_rtcm, int sys, int sync, int *nsat,
                           int *ncell, double *rrng, double *rrate,
                           uint8_t *info, double *psrng, double *phrng,
                           double *rate, double *lock, uint8_t *half,
                           float *cnr)
{
    ...
    /* encode msm header (ref [15] table 3.5-78) */
    setbitu(temp_rtcm->buff,i,12,type       ); i+=12; /* message number */
    setbitu(temp_rtcm->buff,i,12,rtcm->staid); i+=12; /* reference station id */
    setbitu(temp_rtcm->buff,i,30,epoch      ); i+=30; /* epoch time */
    setbitu(temp_rtcm->buff,i, 1,sync       ); i+= 1; /* multiple message bit */
    setbitu(temp_rtcm->buff,i, 3,rtcm->seqno); i+= 3; /* issue of data station */
    setbitu(temp_rtcm->buff,i, 7,0          ); i+= 7; /* reserved */
    setbitu(temp_rtcm->buff,i, 2,0          ); i+= 2; /* clock streering indicator */
    setbitu(temp_rtcm->buff,i, 2,0          ); i+= 2; /* external clock indicator */
    setbitu(temp_rtcm->buff,i, 1,0          ); i+= 1; /* smoothing indicator */
    setbitu(temp_rtcm->buff,i, 3,0          ); i+= 3; /* smoothing interval */
    
    /* satellite mask */
    for (j=0;j<64;j++) {
        setbitu(temp_rtcm->buff,i,1,sat_ind[j]?1:0); i+=1;
    }

    /* signal mask */
    for (j=0;j<32;j++) {
        setbitu(temp_rtcm->buff,i,1,sig_ind[j]?1:0); i+=1;
    }
    /* cell mask */
    for (j=0;j<*nsat*nsig&&j<64;j++) {
        setbitu(temp_rtcm->buff,i,1,cell_ind[j]?1:0); i+=1;
    }
    /* generate msm satellite data fields */
    gen_msm_sat(rtcm,sys,*nsat,sat_ind,rrng,rrate,info);
...
/* encode MSM 4: full pseudorange and phaserange plus CNR --------------------*/
extern int encode_msm4_new(rtcm_t *rtcm,rtcm_t* temp_rtcm, int sys, int sync)
{
    double rrng[64],rrate[64],psrng[64],phrng[64],lock[64];
    float cnr[64];
    uint8_t half[64];
    ...
    /* encode msm header */
    if (!(i=encode_msm_head_new(4,rtcm,temp_rtcm,sys,sync,&nsat,&ncell,rrng,rrate,NULL,psrng,
                            phrng,NULL,lock,half,cnr))) {
        return 0;
    }
    /* encode msm satellite data */
    i=encode_msm_int_rrng(temp_rtcm,i,rrng ,nsat ); /* rough range integer ms */
    i=encode_msm_mod_rrng(temp_rtcm,i,rrng ,nsat ); /* rough range modulo 1 ms */
    
    /* encode msm signal data */
    i=encode_msm_psrng   (temp_rtcm,i,psrng,ncell); /* fine pseudorange */
    i=encode_msm_phrng   (temp_rtcm,i,phrng,ncell); /* fine phase-range */
    i=encode_msm_lock    (temp_rtcm,i,lock ,ncell); /* lock-time indicator */
    i=encode_msm_half_amb(temp_rtcm,i,half ,ncell); /* half-cycle-amb indicator */
    i=encode_msm_cnr     (temp_rtcm,i,cnr  ,ncell); /* signal cnr */
    temp_rtcm->nbit=i;
    return 1;
}