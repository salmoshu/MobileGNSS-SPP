...
static int encode_rtcm3_new(rtcm_t *rtcm,rtcm_t* temp_rtcm, int type, int subtype, int sync)
{
    int ret=0;
    ...
    switch (type) {
        case 1074: ret=encode_msm4_new(rtcm,temp_rtcm,SYS_GPS,sync); break;
        case 1084: ret=encode_msm4_new(rtcm,temp_rtcm,SYS_GLO,sync); break;
        case 1094: ret=encode_msm4_new(rtcm,temp_rtcm,SYS_GAL,sync); break;
        case 1104: ret=encode_msm4_new(rtcm,temp_rtcm,SYS_SBS,sync); break;
        case 1114: ret=encode_msm4_new(rtcm,temp_rtcm,SYS_QZS,sync); break;
        case 1124: ret=encode_msm4_new(rtcm,temp_rtcm,SYS_CMP,sync); break;
        case 1134: ret=encode_msm4_new(rtcm,temp_rtcm,SYS_IRN,sync); break;
    }
    if (ret>0) {
 ...
 * @return int 
 */
static int gen_rtcm3_new(rtcm_t *source_rtcm, rtcm_t* target_rtcm, int type, int subtype, int sync)
{
    uint32_t crc;
    int i=0;

    // trace(4,"gen_rtcm3: type=%d subtype=%d sync=%d\n",type,subtype,sync);
    target_rtcm->nbit=target_rtcm->len=target_rtcm->nbyte=0;

    /* set preamble and reserved */
    setbitu(target_rtcm->buff,i, 8,RTCM3PREAMB); i+= 8;
    setbitu(target_rtcm->buff,i, 6,0          ); i+= 6;
    setbitu(target_rtcm->buff,i,10,0          ); i+=10;

    /* encode rtcm 3 message body */
    if (!encode_rtcm3_new(source_rtcm,target_rtcm,type,subtype,sync)) return 0;

    /* padding to align 8 bit boundary */
    for (i=target_rtcm->nbit;i%8;i++) {
        setbitu(target_rtcm->buff,i,1,0);
    }
    /* message length (header+data) (bytes) */
    if ((target_rtcm->len=i/8)>=3+1024) {
        // trace(2,"generate rtcm 3 message length error len=%d\n",source_rtcm->len-3);
        target_rtcm->nbit=target_rtcm->len=0;
        return 0;
    }
    /* message length without header and parity */
    setbitu(target_rtcm->buff,14,10,target_rtcm->len-3);

    /* crc-24q */
    crc=rtk_crc24q(target_rtcm->buff,target_rtcm->len);
    setbitu(target_rtcm->buff,i,24,crc);

    /* length total (bytes) */
    target_rtcm->nbyte=target_rtcm->len+3;

    return 1;
}

extern uint8_t rtcm_msm7tomsm4(rtcm_t* source_rtcm, int type, rtcm_t* target_rtcm)
{
    if(decode_rtcm3(source_rtcm) >= 0){
        type -= 3;
        gen_rtcm3_new(source_rtcm, target_rtcm, type, 0, !source_rtcm->obsflag);
