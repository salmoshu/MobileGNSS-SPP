...
#include "../user_inc/user_rtklib.h"
...
void update_svr(rtksvr_t *svr, int ret, obs_t *obs, nav_t *nav,
...
int decoderaw(rtksvr_t *svr, int index)
{
    ...
    for (i=0;i<svr->nb[index];i++) {
        ...
        if (svr->format[index]==STRFMT_RTCM2) {
            ...
            /* User code start */
            if (index==0) {
                // copy rover glo eph to base struct to set glo freq
...
void corr_phase_bias(obsd_t *obs, int n, const nav_t *nav)
{
    double freq;
    uint8_t code;
    int i,j;
...
/* periodic command ----------------------------------------------------------*/
void periodic_cmd(int cycle, const char *cmd, stream_t *stream)
{
    const char *p=cmd,*q;
    char msg[1024],*r;
    int n,period;
                       ...
                       solopt_t *solopt, stream_t *moni, char *errmsg)
{
    ...
    if(svr->rtk.opt.stationType == 1){
#ifdef WIN32
            if (!(svr->thread=CreateThread(NULL,0,RoverStationThread,svr,0,NULL))) {
#else
            if (pthread_create(&svr->thread,NULL,RoverStationThread,svr)) {
#endif
                for (i=0;i<MAXSTRRTK;i++) strclose(svr->stream+i);
                sprintf(errmsg,"thread create rover error\n");
                return 0;
            }
    }
    if(svr->rtk.opt.stationType == 2){
#ifdef WIN32
            if (!(svr->thread=CreateThread(NULL,0,BaseStationThread,svr,0,NULL))) {
#else
            if (pthread_create(&svr->thread,NULL,BaseStationThread,svr)) {
#endif
                for (i=0;i<MAXSTRRTK;i++) strclose(svr->stream+i);
                sprintf(errmsg,"thread create base error\n");
                return 0;
            }
    }
    return 1;
