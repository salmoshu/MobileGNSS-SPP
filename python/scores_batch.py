...
if __name__ == '__main__':
    ...
    output_dir = r'.\output\rover.conf'
    pos_files = os.listdir(output_dir)

    if len(target_dirs) != len(pos_files):
        raise ValueError(f"Number of target dirs ({len(target_dirs)}) does not match number of pos files ({len(pos_files)}).")

    for dir, pos_file in zip(target_dirs, pos_files):
        scene, group = dir.split(slash_type)[-2:]
