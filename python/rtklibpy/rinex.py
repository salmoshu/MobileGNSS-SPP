    ...
    def decode_nav(self, navfile, nav):
        ...
        with open(navfile, 'rt') as fnav:
            ...
            for line in fnav:
                ...
                if sys == uGNSS.GPS or sys == uGNSS.QZS or sys == uGNSS.GAL:
                    eph = Eph(sat)
                    eph.toc = toc
                    ...
                    nav.eph.append(eph)
                elif sys == uGNSS.BDS: # not finished 
                    eph = Eph(sat)
                    eph.toc = gn.bdt2gpst(toc)
                    eph.f0 = self.flt(line, 1)
                    eph.f1 = self.flt(line, 2)
                    eph.f2 = self.flt(line, 3)
                    ...
                    eph.svh = int(self.flt(line, 1))
                    tgd = np.zeros(2)
                    tgd[0] = float(self.flt(line, 2))  # TGD1 B1/B3                    
                    tgd[1] = float(self.flt(line, 3))  # TGD2 B2/B3
                    eph.tgd = tgd
                    ...
                    eph.toe = gn.bdt2gpst(gn.bdt2time(eph.week, eph.toes))
                    eph.tot = gn.bdt2gpst(gn.bdt2time(eph.week, tot))
                    nav.eph.append(eph)
                else:  # GLONASS
                    if prn > uGNSS.GLOMAX:
