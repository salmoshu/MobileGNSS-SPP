...
pmode = 'single'         # single, static, kinematic
filtertype = None        # forward, backward, combined, combined_noreset
use_sing_pos = True      # run initial single precision sol each epoch, not
                         # necessary unless receiever clock errors are large
elmin = 15               # minimum elevation for float solution (degrees)
cnr_min = [25, 20]       # min signal strength [freq1, freq2] (dB-Hz)
...
efact = {uGNSS.GPS: 1.0, uGNSS.GLO: 1.5, uGNSS.GAL: 1.0, uGNSS.BDS: 1.5, uGNSS.QZS: 1.0} # relative weighting of each constellation
err = [0, 0.003, 0.000, 0.0, 0.003, 0, 5e-12, 1.0]  # err sigmas [-, base, el, bl, snr, rcvstd, satclk, doppler]
...
sig_tbl = {'1C': rSIG.L1C, '1X': rSIG.L1X, '1W': rSIG.L1W,
           '2W': rSIG.L2W, '2L': rSIG.L2L, '2X': rSIG.L2X, '2I': rSIG.L1I,
           '5Q': rSIG.L5Q, '5X': rSIG.L5X, '6I': rSIG.L6I, '7Q': rSIG.L7Q,
           '7X': rSIG.L7X}
...
freq_ix1 = {uGNSS.GPS: 2, uGNSS.GLO: 5, uGNSS.GAL: 2, uGNSS.BDS: 2, uGNSS.QZS: 2} # L5

# ---------- Frequencies currently supported-------------
freq = [1.57542e9,   # L1/E1
        1.22760e9,   # L2
        1.17645e9,   # L5/E5a/B2a
        1.20714e9,   # E5b/B2I/B2b
        1.60200e9,   # G1
        1.24600e9,   # G2
        1.561098e9,  # BDS B1I
        1.26852e9]   # BDS B3
